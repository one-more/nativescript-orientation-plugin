 ...
 */
orientation.addOrientationApplier = function(
    newOrientationApplier,
) {
 ...
 */
orientation.removeOrientationApplier = function(
    orientationApplier,
) {
    if (!orientationApplier) return;

    orientationAppliers = orientationAppliers.filter(function(
        oa,
    ) {
...
var applyOrientationToPage = function(page, args) {
    ...
    var isLandscape =
        currentOrientation === enums.DeviceOrientation.landscape;
    if (!args || !args.force) {
        var containsLandScape = page.classList.contains(
            'landscape',
        );
    ...
    if (
        page.exports &&
        typeof page.exports.orientation === 'function'
    ) {
...
if (global.android) {
    ...
    orientation.setFullScreen = function(setFullScreen) {
        ...
        app.setStatusBarHiddenWithAnimation(
            fullScreen,
